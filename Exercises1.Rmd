...
ggplot(data)+geom_line(aes(x=Month, y=mean))+ggtitle("Best Time Of Year To Fly To Minimize Delays For Atlanta")
```
...
ggplot(data)+geom_line(aes(x=Month, y=mean))+ggtitle("Best Time Of Year To Fly To Minimize Delays For Log Angeles")
```
...
ggplot(data)+geom_line(aes(x=Month, y=mean))+ggtitle("Best Time Of Year To Fly To Minimize Delays For Chicago")
```
...
ggplot(data)+geom_line(aes(x=Month, y=mean))+ggtitle("Best Time Of Year To Fly To Minimize Delays For Dallas")
```
...
ggplot(data)+geom_line(aes(x=Month, y=mean))+ggtitle("Best Time Of Year To Fly To Minimize Delays For Denver")
```
...
ggplot(data)+geom_line(aes(x=Month, y=mean))+ggtitle("Best Time Of Year To Fly To Minimize Delays For New York")
```
...
knitr::kable(c("min_k=84", "min_rmse=7432.471"))
```

From the above result, the optimal K is 84 because of the minimum RMSE on it. The prediction is in the following.

```{r, echo=FALSE,fig.width =5, fig.height = 2,fig.align='center'}
knn84 = knnreg(price ~ mileage, data=data1, k=84)
data350_test = data350_test %>%
 mutate(price_pred1 = predict(knn84, data350_test))
# p_test = ggplot(data = data350_test) + geom_point(mapping = aes(x = mileage, y = price), alpha=0.2)
...
knitr::kable(c("min_k=15", "min_err=9971.221"))
```
From the above result, the optimal K is 15 because of the minimum RMSE on it. The prediction is in the following.
...
#ggplot(rmse_AMG, aes(k))+geom_line(aes(y=rmse), color = "tomato")+ggtitle("RMSE/K of 65AMG")+theme(plot.title = element_text(hjust = 0.5))
# knitr::kable(rmse_AMG[rmse_AMG$rmse==min(rmse_AMG$rmse),])

# Plot created plot
knitr::include_graphics("./graphs/5.png")
knitr::kable(c("min_k=17", "rmse=14255.92"))
```

From the above result, the optimal K is 17 because of the minimum RMSE on it. The prediction is in the following.

```{r, echo=FALSE,fig.width =5, fig.height = 2,fig.align='center'}
# Then for the optimal value of K, show a plot of the fitted model, i.e. predictions vs. x. 
knn17 = knnreg(price ~ mileage, data=data2, k=54)
dataAMG_test = dataAMG_test %>%
 mutate(price_pred2 = predict(knn17, dataAMG_test))
...
knitr::kable(c("min_k=13", "min_err=20717.91"))
```
...
The optimal value of k of 350 is larger than that of 65 AMG. Because the number of observations for 350 is 416, which is larger than that or observations for 65 AMG (which is 292). That means the larger number of observation need larger K to balance between its Bias and Variance.
